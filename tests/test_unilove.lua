...
T['codepoint_positions()'] = new_set({
    parametrize = {
        ...
        -- Emoji are just multi byte codepoints to the iterator: modifiers, ZWJ
        -- sequences and flags form graphemes, but that is `grapheme_at`'s
        ...
        -- Invalid input follows the same byte level rules as we have done with
...
T['codepoints()'] = new_set({
    parametrize = {
        ...
        -- Invalid lead bytes are reported as their own values, losing nothing
...
-- Rows for `grapheme_at()`: point cases first, and then one row per byte of the
-- multi byte graphemes, to check the "at every byte" property while keeping the
-- failures pinpointing the exact column.
